    ...
    def verify_master_password(self):
        ...
        try:
            ...
            if not os.path.exists(self.DB_FILE):
                raise Exception(
                    "Password manager not initialized or database corrupted."
                )
    ...
    def save_password(self, address, password):
        ...
        try:
            ...
            entries = [
                e
                for e in entries
                if self._decrypt_entry(e, key).split(",")[0] != address
            ]
...
def main():
    ...
    group.add_argument(
        "--put",
        nargs=3,
        metavar=("MASTER_PASSWORD", "ADDRESS", "PASSWORD"),
        help="Store password for an address",
    )
    group.add_argument(
        "--get",
        nargs=2,
        metavar=("MASTER_PASSWORD", "ADDRESS"),
